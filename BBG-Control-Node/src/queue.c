...
int queue_init(void)
{
	...
	attr.mq_flags = 0;
	attr.mq_maxmsg = 20;
	attr.mq_msgsize = sizeof(logger_struct); //Change afterwards
	attr.mq_curmsgs = 0;
	...
	attr.mq_msgsize = sizeof(struct packet_struct);
	packet_mq = mq_open(PACKET_QUEUE, O_RDWR | O_CREAT, 0644, &attr);
	if (packet_mq == -1)
	{
		perror("Packet message queue initialization failed.\n");
		/*Closing all the previous resources and freeing memory uptil failure*/
		mq_close(heartbeat_mq);
		mq_unlink(HEARTBEAT_QUEUE);
		mq_close(log_mq);
 ...
 */
void queue_send(mqd_t mq, logger_struct data_send, uint8_t loglevel, uint8_t prio)
{
	if (loglevel & g_ll)
	{
		ssize_t res;
		res = mq_send(mq, (char *)&data_send, sizeof(logger_struct), prio);
		if (res == -1)
		{
 ...
 * @return logger_struct - The structure local onject in which the data received is stored.
 */
logger_struct queue_receive(mqd_t mq)
{
	logger_struct data_rcv;
	ssize_t res;
	res = mq_receive(mq, (char *)&data_rcv, sizeof(logger_struct), NULL);
	if (res == -1)
	{
...
err_t queues_close(void)
{
	...
	if (mq_close(packet_mq))
	{
		perror("ERROR: mq_close(packet); in queues_close() function");
	}
	return OK;
}
...
err_t queues_unlink(void)
{
	...
	if (mq_unlink(PACKET_QUEUE))
	{
		perror("ERROR: mq_unlink(packet); in queues_unlink() function");
	}
}
